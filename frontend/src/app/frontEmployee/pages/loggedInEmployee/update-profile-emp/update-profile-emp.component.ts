...
@Component({
  selector: 'app-update-profile-emp',
  templateUrl: './update-profile-emp.component.html',
  styleUrls: ['./update-profile-emp.component.css']
})
export class UpdateProfileEmpComponent implements OnInit {
  userProfile: User = {
    userId: 0,
    username: '',
    password: '',
    ...
    this.userService.updateUser(this.userProfile.userId, this.userProfile).subscribe(
      () => {
        Swal.fire({
          ...
          this.router.navigate(['/ProfileEmp']);
        }).then(() => {
